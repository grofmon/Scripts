...
# CHANGELOG
# 05 AUGUST 2012
#  - ADDED SUPPORT FOR GETTING WEATHER WARNING DATA
# 21 APRIL 2012
...
class Forecasts:
    ...
    def getData(self,opts):
        ...
        if self.url != '':
            ...
            print "*** NO URL CREATED, PLEASE CALL setURL() PRIOR TO CALLING getData() ***"
        
        # DETERMINE IF WE ARE GRABBING HAZARD DATA
        if opts.wxwarnings:
            urld = urllib2.urlopen(self.hazurl)
            ...
            # SET THE CLASS VARIABLE FOR JSON DATA
            self.jsondatahazard = json.loads(lines)
            if opts.debug:
                print "\n** actual jsondata dictionary"
                print self.jsondatahazard
