...
# CHANGELOG
# 05 AUGUST 2012
#  - STARTED ADDING SUPPORT FOR WEATHER WARNING INFORMATION, ONLY GRABS URL FOR FIRST PART OF DATA
# 03 AUGUST 2012
#  - ADDED SUPPORT FOR PRINTING THE DEGREE SYMBOL, REQUIRES GEEKLET TO BE SET TO UNICODE
# 04 MAY 2012
#  - FIXED BUG IN FORECAST PRINTING WHERE THE HIGH AND LOW WERE BACKWARDS
#    (THANKS TO TAXIDRIVER ON THE MACRUMORS FORUM)
# 21 APRIL 2012
...
class WeatherUnderground(forecasts.Forecasts):
    ...
    def setURL(self,opts):
        ...
        if opts.wxwarnings:
            # FIRST WE NEED TO GET THE PORTION OF THE PAGE THAT WILL CONTAIN THE LINK TO THE STUFF
            ender = ' and xpath=\'//div[@class="alertBox br5"]\''
            self.hazurl = YQLBASEURL + "?q=" + myencode(yqlquery+ender) + "&format=" + YQLFORMAT1
            if opts.debug:
                print "\n*** wunderground url creation - hazards/warnings"
                print yqlquery
                print self.hazurl
                print DEBUGSPACER
    ...
    def parseData(self,opts):
        ...
        if   opts.wundergroundfcasttype == 'current':
            ...
            # WORK TO GET THE WEATHER WARNINGS, THIS IS A MULTIPAGE PROCESS
            #if opts.wxwarnings:
        ...
        if   opts.wundergroundfcasttype == 'current':
            ...
            if opts.orientation == 'vertical':
                ...
                for i in xrange(opts.wundergroundnumdays):
                    ...
                    tmp = mystr2 % (self.forecasts[i].high,self.units['temperature'])
                    highlist.append(tmp)
                    tmp = mystr3 % (self.forecasts[i].low,self.units['temperature'])
                    lowlist.append(tmp)
