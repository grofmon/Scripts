    ...
    def printForecasts(self, opts):
        ...
        if   opts.yahoofcasttype == 'current':
            ...
            if opts.currentoutputtype == 'detailed':
                if opts.degreesymbol:
                    u = DEGREE+self.units['temperature']
                    print "H: %s%2s L: %s%2s" % (self.forecasts[0].high,u,self.forecasts[0].low,u)
                else:
            ...
            for fcast in self.forecasts:
                print mystr % (fcast.day,fcast.date,fcast.condition,fcast.high,u,fcast.low,u)
            ...
            if opts.degreesymbol:
                u = DEGREE+tempunit
                mystr = "%s: %s\nHigh: %3s%2s Low:  %3s%2s"
            else:
                u = tempunit
                mystr = "%s: %s\nHigh: %3s%1s Low:  %3s%1s"
            for fcast in self.forecasts:
                print mystr % (fcast.day,fcast.condition,fcast.high,u,fcast.low,u)
