...
cfg_dir=/ccshare/linux/c_files/evtc/daily_cs
...
#with bcc            cat $tmp_file | mailx -n -s "Attention: EVTC config spec updated ($datesubj)" -r "$mailfr" -b "$mailbcc" "$mailto"
            cat $tmp_file | mailx -n -s "Attention: EVTC config spec updated ($datesubj)" -r "$mailfr" "$mailto"
        else
#            cat $tmp_file | mail -s "Attention: EVTC config spec updated ($datesubj)" "$mailto" -b "$mailbcc" -- -f "$mailfr"
            cat $tmp_file | mail -s "Attention: EVTC config spec updated ($datesubj)" "$mailto" -- -f "$mailfr"
        fi
